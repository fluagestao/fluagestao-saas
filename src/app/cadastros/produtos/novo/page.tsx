...
import type { CatalogoRow, CategoriaRow, EtiquetaRow } from "@/components/admin/tipos";
...
export default async function NovoProdutoPage({
  ...
  let draft: { id: string; sku: string; slug: string; rascunho: boolean } | null = null;
  ...
  if (params.draft) {
    ...
    if (data?.rascunho) {
      draft = { id: data.id, sku: data.sku, slug: data.slug, rascunho: true };
    }
  }

  if (!draft) {
    const novo = await prepararNovoProduto();
    redirect(`/cadastros/produtos/novo?draft=${novo.id}`);
  }
