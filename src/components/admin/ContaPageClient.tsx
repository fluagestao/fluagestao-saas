...
const TITULOS: Record<ContaSecao, { titulo: string; descricao: string }> = {
  empresa: {
    ...
    descricao: "Mantenha os dados principais da sua empresa atualizados dentro da Flua.",
  },
  plano: {
    ...
    void (async () => {
      ...
      try {
        ...
        if (empresaData) {
          ...
          setNome(empresaAtual.name ?? companyName);
          setDocumento(empresaAtual.document ?? "");
          setTelefone(empresaAtual.phone ?? "");
        ...
        <header className="sticky top-0 z-40 border-b border-[var(--admin-border)] bg-white/96 backdrop-blur-xl">
          <div className="mx-auto flex h-[74px] max-w-[1540px] items-center justify-between gap-4 px-4 sm:px-6 xl:px-8">
            <div className="flex items-center gap-4">
              ...
              <Link
                href="/admin"
                className="hidden items-center gap-2 rounded-xl px-3 py-2 text-sm font-medium text-[var(--admin-ink-soft)] transition hover:bg-[var(--cream-soft)] sm:inline-flex"
              >
            ...
            <div className="flex min-w-0 items-center gap-2">
              ...
              <span className="hidden min-w-0 sm:block">
                <strong className="block max-w-40 truncate text-xs font-semibold">{companyName}</strong>
                <span className="block max-w-40 truncate text-[10px] text-[var(--admin-muted)]">{displayName || email}</span>
              </span>
            </div>
          </div>
        </header>
            ...
            <aside className="h-fit rounded-2xl border border-[var(--admin-border)] bg-white p-2 shadow-sm lg:sticky lg:top-[94px]">
              {NAVEGACAO.map((item) => {
                ...
                return (
                  <Link
                    key={item.id}
                    href={`/admin/conta/${item.id}`}
                    className={`flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-medium transition ${
                      ativo
                        ? "bg-[var(--cream)] text-[var(--wine)]"
              ...
              {carregando && secao !== "usuarios" ? (
                ...
                <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_300px]">
                  ...
                  <aside className="rounded-2xl border border-[var(--admin-border)] bg-white p-5">
                    ...
                    <h2 className="mt-4 text-base font-semibold">{empresa?.name || companyName}</h2>
                    ...
                    <div className="mt-5 rounded-xl bg-[var(--cream-soft)] p-3 text-xs leading-5 text-[var(--admin-ink-soft)]">
                      Esses dados identificam sua empresa dentro da plataforma e são usados pelos módulos da Flua.
                    </div>
                  </aside>
                </div>
              ) : secao === "plano" ? (
                  ...
                  <div className="rounded-2xl border border-[var(--admin-border)] bg-white p-6">
                    ...
                    <div className="mt-7 grid gap-3 sm:grid-cols-2">
                      ...
                      <div className="rounded-xl bg-[var(--cream-soft)] p-4">
                        ...
                        <strong className="mt-1 block truncate text-sm">{companyName}</strong>
                      </div>
                    </div>
                  </div>
                ...
                <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
                  ...
                  <aside className="rounded-2xl border border-[var(--admin-border)] bg-white p-5">
                    ...
                    <p className="mt-2 text-sm text-[var(--admin-muted)]">{displayName || "Usuário"}</p>
